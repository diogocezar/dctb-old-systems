		...
		while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
			...
			for($i=0; $i<$qtdCampos; $i++){
				...
				$template->setCurrentBlock("bloco_detalhe");
					...
					$valorDetalhe =  strtoupper($dados[$camposMap[$tabela][$i]]);
					$template->setVariable("cmp_detalhe_valor", limitaStr($valorDetalhe, 80));
	...
	if(!DB::isError($resultado)){
		...
		$template->loadTemplatefile($templateHtmlName, true, true);
			...
			while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
				...
				$item = "<span onmouseover=\"call_registerDetais('$pk')\" class=\"cursorLink\" title=\"{$dados[$campo]}\">".limitaStr($dados[$campo], 80)."</span>";
