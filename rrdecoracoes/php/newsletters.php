...
define(ASSUNTO, "newsletters");
define(TABELA, $tabela['novidades']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'tituloNovidade');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERAL);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idNovidade, tituloNovidade FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "newsletter.php?acao=atualizar&id={$dados['idNovidade']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=newsletter&id={$dados['idNovidade']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['tituloNovidade'], LIMITE_GENRENCIAR);
