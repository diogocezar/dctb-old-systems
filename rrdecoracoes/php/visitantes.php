...
define(ASSUNTO, "visitantes");
define(TABELA, $tabela['visitantes']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeVisitante');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERAL);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idVisitante, nomeVisitante FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "cadastro.php?acao=atualizar&id={$dados['idVisitante']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=visitante&id={$dados['idVisitante']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeVisitante'], LIMITE_GENRENCIAR);
