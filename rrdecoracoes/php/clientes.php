...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeCliente');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERAL);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idCliente, nomeCliente FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "cliente.php?acao=atualizar&id={$dados['idCliente']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=cliente&id={$dados['idCliente']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeCliente'], LIMITE_GENRENCIAR);
