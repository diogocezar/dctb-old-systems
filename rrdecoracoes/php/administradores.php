...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeAdministrador');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERAL);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idAdministrador, nomeAdministrador FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "administrador.php?acao=atualizar&id={$dados['idAdministrador']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=admin&id={$dados['idAdministrador']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeAdministrador'], LIMITE_GENRENCIAR);
