...
define(ASSUNTO, "tipos de ecoturismos");
define(TABELA, $tabela['tipoecoturismo']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeTipoEcoturismo');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idTipoEcoturismo, nomeTipoEcoturismo FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "tipoEcoturismo.php?acao=atualizar&id={$dados['idTipoEcoturismo']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=tipoEcoturismo&id={$dados['idTipoEcoturismo']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeTipoEcoturismo'], LIMITE_GENRENCIAR);
