...
if(empty($tipo)){
	echo "<script language=javascript>alert('Você deve selecionar um tipo de ecoturismo!');location.href='index.php'</script>";
}
...
define(ASSUNTO, "ecoturismos");
define(TABELA, $tabela['ecoturismo']);

/* Contando registros */
$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." WHERE idTipoEcoturismo = $id");
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeEcoturismo');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idEcoturismo, nomeEcoturismo, descricaoEcoturismo FROM ".TABELA." WHERE idTipoEcoturismo = $id ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Linha a ser impressa */
			$nome = "<a href = \"vejaEcoturismo.php?id={$dados['idEcoturismo']}\">".limitaStr($dados['nomeEcoturismo'], LIMITE_GENRENCIAR)."</a>";
			$descricao = limitaStr($dados['descricaoEcoturismo'], LIMITE_DESCRICAO);
...
if(TOTAL > 1){	
	...
	$paginacao .= "<a href=\"".PAGINA_ATUAL."?start=0&id=$id\"><img src=\"../images/primeiro.jpg\" border=\"0\" align=\"absmiddle\" /><a>&nbsp;&nbsp;";
	if($pag > 1){
		$paginacao .= "<a href=\"".PAGINA_ATUAL."?start=$mostra_ante&pag=$pag_ante&id=$id\">";
	}		
	...
	for($i=$inicioMostra; $i<=$fimMostra; $i++){
		...
		if(POR_PAGINA*($i-1) == ATUAL){	
			...
			$aevi = POR_PAGINA*($i-1);
			$paginacao .= " <a href=\"".PAGINA_ATUAL."?start=$aevi&pag=$i&id=$id\" class=\"linkPaginacao\">$i</a> ";
		}
	}
	
	$aevi = POR_PAGINA*(TOTAL-1);
	$i    = TOTAL;
	$link = PAGINA_ATUAL."?start=$aevi&pag=$i&id=$id";
	
	$paginacao .= "&nbsp;<b>.</b>.&nbsp;";
	
	if($pag < TOTAL){						
		$paginacao .= "<a href=\"".PAGINA_ATUAL."?start=$mostra_prox&pag=$pag_prox&id=$id\">";
	}
