...
define(ASSUNTO, "tipos de atividades");
define(TABELA, $tabela['tipoatividade']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeTipoAtividade');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idTipoAtividade, nomeTipoAtividade FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "tipoAtividade.php?acao=atualizar&id={$dados['idTipoAtividade']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=tipoAtividade&id={$dados['idTipoAtividade']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeTipoAtividade'], LIMITE_GENRENCIAR);
