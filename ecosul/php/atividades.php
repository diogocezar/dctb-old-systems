...
if(empty($tipo)){
	echo "<script language=javascript>alert('Você deve selecionar um tipo de atividade!');location.href='index.php'</script>";
}
...
/* Contando registros */
$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." WHERE idTipoAtividade = $id");
...
define(SQL, "SELECT idAtividades, nomeAtividade, descricaoAtividade FROM ".TABELA." WHERE idTipoAtividade = $id ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Linha a ser impressa */
			$nome = "<a href = \"vejaAtividade.php?id={$dados['idAtividades']}\">".limitaStr($dados['nomeAtividade'], LIMITE_GENRENCIAR)."</a>";
			$descricao = limitaStr($dados['descricaoAtividade'], LIMITE_DESCRICAO);
