...
define(ASSUNTO, "cidades");
define(TABELA, $tabela['cidades']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeCidade');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idCidade, nomeCidade FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "cidade.php?acao=atualizar&id={$dados['idCidade']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=cidade&id={$dados['idCidade']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeCidade'], LIMITE_GENRENCIAR);
