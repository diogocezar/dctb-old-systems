...
class Pikture{
	...
	function firstPicture($dir){
	  ...
	  * @return void
	  */  
	function showPhotos($subDir, $page, $scalar){
		$this->folder = $subDir;
		...
		for($i=0; $i<$linhas; $i++){
			...
			for($j=0; $j<$colunas; $j++){
				...
				$retorno .= "<a href=\"#\" onClick=\"abrir('showImg.php?loc=".$dirFoto."&a=$altura&l=$largura&titulo=".$nome."', '".$largura."', '".$altura."', 'no');\" >";
				$retorno .= "<img src=\"img.php?loc=".$dirFoto."&s=$scalar&l=107&a=90\" border=\"0\">";
				$retorno .= "<br>";
				$retorno .= "</a>";
				$countFotos++;
				$retorno .= "</td>";
