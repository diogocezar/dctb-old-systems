...
define(ASSUNTO, "fabricantes");
define(TABELA, $tabela['fabricantes']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'fab_nome');
define(ATUAL, $start);
define(POR_PAGINA, PP_FABRICANTES);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT fab_nome, fab_cod FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "fabricante.php?acao=atualizar&id={$dados['fab_cod']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=fabricante&id={$dados['fab_cod']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['fab_nome'], LIMITE_GENRENCIAR);
...
$template->setCurrentBlock("bloco_gerenciar");
	...
	/* Banner */
		$template->setVariable("banner", "topAdminFabricantes.gif");	

	/* Titulos */
		$template->setVariable("tituloGerenciar", "Gerenciar Fabricantes");
