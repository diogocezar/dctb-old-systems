...
define(ASSUNTO, "dicas");
define(TABELA, $tabela['dicas']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'dic_titulo');
define(ATUAL, $start);
define(POR_PAGINA, PP_LOJAS);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT dic_cod, dic_titulo FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "admDica.php?acao=atualizar&id={$dados['dic_cod']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=dica&id={$dados['dic_cod']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['dic_titulo'], LIMITE_GENRENCIAR);
