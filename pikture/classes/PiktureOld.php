...
class Pikture{
	...
	function arrayFolders(){
	  ...
	  * @return String
	  */  
	function printFolders($pagMostra, $varGet, $iconePastinha){
		$array = $this->arrayFolders();
		for($i=0; $i<count($array); $i++){
			if($iconePastinha == "primeira_foto"){
				$iconePrimeira = $this->firstPicture($this->diretorio.'/'.$array[$i]);
				$pastinha = "<img src=\"img.php?loc=".$iconePrimeira."&t=0&s=false\" align=\"absmiddle\">"; 
			}
			else{
				$pastinha = "<img src=\"$iconePastinha\" align=\"absmiddle\">"; 
			}
			$link      = "<a href = \"$pagMostra?$varGet={$array[$i]}\">";
			$fechaLink = "</a>";
			$retorno  .= $pastinha.' '.$link.$array[$i].$fechaLink." {<strong>".$this->countPhotos($this->diretorio.'/'.$array[$i])." Fotos</strong>}";
			$retorno  .= " # [<b> ".number_format($this->dirSize($this->diretorio.'/'.$array[$i]), 3, '.', '.')."</b> MB ]";
			$retorno  .= "<br>";
		}
		return $retorno;
	}
	...
	function firstPicture($dir){
		...
		return $this->diretorio.'/'.$dir.'/'.$dirScan[0];
	}
	...
	function showPhotos($subDir, $page, $scalar, $printFileName, $printSize){
		...
		$retorno .= "<table width=\"100%\" border=\"0\" cellpadding=\"5\" cellspacing=\"5\">";
		for($i=0; $i<$linhas; $i++){
			$retorno .= "<tr>";
			if($i+1 == $linhas){
				if($sobra > 0){
			...
			for($j=0; $j<$colunas; $j++){
				...
				$retorno .= "<a href=\"javascript:abrir('mostra.php?loc=".$dirFoto."&t=1&a=$altura&l=$largura&titulo=".$fotos[$countFotos]."', '".$largura."', '".$altura."', 'no');\">";
				$retorno .= "<img src=\"img.php?loc=".$dirFoto."&t=1&s=$scalar\" border=\"0\">";
				$retorno .= "<br>";
				if($printFileName){
					$retorno .= "$fotos[$countFotos]";
				}
				if($printSize){
					$retorno .= number_format($this->photoSize($dirFoto), 3, ',', '.');
				}
				$retorno .= "</a>";
				$this->setStyle('fecha');				
				$countFotos++;
