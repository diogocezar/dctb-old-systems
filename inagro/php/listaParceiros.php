...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			$campoPreenche = $dados[$objeto->campos[1]];

			if(!empty($campoPreenche)){
				$foto = $campoPreenche;
				$template->setVariable("img", "<img src=\"img.php?l=119&a=93&s=no&loc=".$foto."\" border=\"0\"></a>");
			}
			else{
			...
			$titulo    = limitaStr($dados[$objeto->campos[2]], $limiteTitulo);
			$descricao = limitaStr($dados[$objeto->campos[3]], $limiteResumo);
			
			$linkPar = $dados[$objeto->campos[3]];
			
			if(!eregi($linkPar, 'http://')) $linkPar = 'http://'.$linkPar;
			
			$template->setVariable("titulo", $titulo);
			$template->setVariable("descricao", "<a href=\"$linkPar\" class=\"link_preto\" target=\"_blank\">".$descricao."</a>");
