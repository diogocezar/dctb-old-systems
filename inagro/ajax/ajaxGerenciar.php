...
function deleteRegister($tabela, $key){
	...
	/* Chamando função para excluir fotos */
	$objDel = $controlador[$tabela];
	$objDel->__toFillGeneric();
	$objDel->__get_db($key);
	$objDel->deletePictures();
	...
	if(!DB::isError($resultado)){
		...
		$template->loadTemplatefile($templateHtmlName, true, true);
			...
			while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
				...
				$template->setVariable("cmp_alterar", "<a href=\"frm".ucfirst($tabela).".php?acao=atualizar&id=$pk\" class=\"link_gen\">Alterar</a>");
