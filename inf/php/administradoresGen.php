...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeAd');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idAdministradores, nomeAd FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "administrador.php?acao=atualizar&id={$dados['idAdministradores']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=admin&id={$dados['idAdministradores']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeAd'], LIMITE_GENRENCIAR);
