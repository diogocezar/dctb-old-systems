...
define(ASSUNTO, "eventos");
define(TABELA, $tabela['eventos']);
...
define(ORDEM, 'DESC');
define(ORDENADO, 'dataEv');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idEventos, descricaoEv, dataEv FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "evento.php?acao=atualizar&id={$dados['idEventos']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=evento&id={$dados['idEventos']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['descricaoEv'], LIMITE_GENRENCIAR)." (<b>".desconverteData($dados['dataEv'])."</b>)";
