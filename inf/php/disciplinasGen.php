...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeDi');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idDisciplinas, nomeDi FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "disciplina.php?acao=atualizar&id={$dados['idDisciplinas']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=disciplina&id={$dados['idDisciplinas']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['nomeDi'], LIMITE_GENRENCIAR);
