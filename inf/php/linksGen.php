...
define(ASSUNTO, "links");
define(TABELA, $tabela['links']);
...
define(ORDEM, 'ASC');
define(ORDENADO, 'nomeLi');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idLinks, nomeLi, urlLi FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "link.php?acao=atualizar&id={$dados['idLinks']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=link&id={$dados['idLinks']}");
		
		/* Linha a ser impressa */
			$link = replaceLink($dados['urlLi']);
			$linha = limitaStr($dados['nomeLi'], LIMITE_GENRENCIAR)." <<a href = \"$link\" class=\"linkPaginacao\">$link</a>>";
