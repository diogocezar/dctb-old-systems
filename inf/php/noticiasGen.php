...
define(ORDEM, 'DESC');
define(ORDENADO, 'dataHoraNo');
define(ATUAL, $start);
define(POR_PAGINA, PP_GERENCIAR);
define(TOTAL, ceil((QTD)/POR_PAGINA));
define(SQL, "SELECT idNoticias, tituloNo FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Links da imagens dos Campos */
			$template->setVariable("linkEditar", "noticia.php?acao=atualizar&id={$dados['idNoticias']}");
			$template->setVariable("linkExcluir", "exclui.php?tipo=noticia&id={$dados['idNoticias']}");
		
		/* Linha a ser impressa */
			$linha = limitaStr($dados['tituloNo'], LIMITE_GENRENCIAR);
