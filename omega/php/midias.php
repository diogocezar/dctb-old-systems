...
if($permitido != true){
	...
	$templateHtmlName = 'gerenciarMid.html';
	...
	define(ASSUNTO, "mídias");
	define(TABELA, $tabela['midia']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'mid_cod');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_MIDIA);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT mid_cod, fil_cod, mid_cod_controle FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "midia.php?acao=atualizar&id={$dados['mid_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=midia&id={$dados['mid_cod']}");
				$template->setVariable("linkLocar", "alteraMidia.php?id={$dados['mid_cod']}&altera=locar");
				$template->setVariable("linkDisponibilizar", "alteraMidia.php?id={$dados['mid_cod']}&altera=disponibilizar");
			
			/* Linha a ser impressa */
				$linha = retornaNomeFilme($dados['fil_cod']);
				$template->setVariable("linha", limitaStr($linha, LIMITE_GENRENCIAR)." (<b> ".$dados['mid_cod_controle']."</b> )");
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Mídias");
