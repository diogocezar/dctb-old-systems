...
if($permitido != true){
	...
	$template->setCurrentBlock("bloco_newsletter");
		...
		/* Formulario */
			...
			$template->setVariable("actionNews", "newsletter.php");
		
		/* Titulos */
			$template->setVariable("tituloNewsLetter", "Envie novidades para todos os cadastrados");
	
		/* Nomes dos Campos */
			$template->setVariable("campoNewsletter", "news");
		
		/* BB Code */
			$template->setVariable("onClickNegrito", "wrapSelection(form_news.news,'[B]','[/B]')");
			$template->setVariable("onClickItalico", "wrapSelection(form_news.news,'[I]','[/I]')");
			$template->setVariable("onClickSublinhado", "wrapSelection(form_news.news,'[U]','[/U]')");
			$template->setVariable("onClickImg", "wrapSelectionWithImg(form_news.news)");
			
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "enviaForm(form_news)");
