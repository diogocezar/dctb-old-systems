...
if($permitido != true){
	...
	define(SQL, "SELECT fil_cod, mid_cod_controle
	             FROM {$tabela['midia']}
				 WHERE mid_status = 'Sim'
	...
	if(!$contem_resultado){
		$template->setCurrentBlock("bloco_detalhes_item");
				$template->setVariable("linha", "Nenhuma mídia locada no momento.");
		$template->parseCurrentBlock("bloco_detalhes_item");
	}
	...
	$template->setCurrentBlock("bloco_detalhes");
		...
		/* Titulos */
			$template->setVariable("tituloDetalhes", "Mídias Locadas");
