...
if($permitido != true){
	...
	define(ASSUNTO, "diretores");
	define(TABELA, $tabela['diretor']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'dir_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CLASSIFICACAO);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT dir_cod, dir_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "diretor.php?acao=atualizar&id={$dados['dir_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=diretor&id={$dados['dir_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", limitaStr($dados['dir_nome'], LIMITE_GENRENCIAR));
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Categorias");
