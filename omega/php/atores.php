...
if($permitido != true){
	...
	define(ASSUNTO, "atores");
	define(TABELA, $tabela['ator']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'ato_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_ATORES);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT ato_cod, ato_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "ator.php?acao=atualizar&id={$dados['ato_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=ator&id={$dados['ato_cod']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"mostraAtor.php?id={$dados['ato_cod']}\" target=\"_blank\">";
				$linha .= limitaStr($dados['ato_nome'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Atores");
