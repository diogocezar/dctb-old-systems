...
if($_SESSION['usuarioSession'] == 'sim'){
	...
	$nomeUser = retornaNomeCliente($codUser);
	$permitido = true;
}
...
if($permitido != true){
	...
	/* Capturando Pedido */
	$templateHtmlName = 'minhaConta.html';
	...
	/* Bloco Titulo */
		$template->setCurrentBlock("bloco_titulo");
			$template->setVariable("bemVindo", "Bem vindo(a), <b>".$nomeUser."</b>");
			$template->setVariable("linkSair", "logout.php");
			$template->setVariable("altSair",  "Sair !");
		$template->parseCurrentBlock("bloco_titulo");
		
	/* Bloco Minha Conta */
		$template->setCurrentBlock("bloco_minha_conta");
			$template->setVariable("linkAlterar", "cadastrese.php?acao=atualizar");
			$template->setVariable("altAletrar", "Alterar Dados !");
			$template->setVariable("linkVerFilmes", "listaFimes.php");
			$template->setVariable("altVerFilmes", "Filmes Locados !");
			$template->setVariable("linkVerLoc", "listaLocacoes.php");
			$template->setVariable("altVerLoc", "Locações Anteriores !");
			$template->setVariable("linkVerFav", "listaFavoritos.php");
			$template->setVariable("altVerFav", "Favoritos !");
		$template->parseCurrentBlock("bloco_minha_conta");
			
	$show = $template->get();
	
	/* Título da Página Interna */
	$tituloInterna = "Minha Conta";
