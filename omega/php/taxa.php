...
if($permitido != true){
	...
	switch($acao){
		case 'adicionar' :
			$taxa['action'] = "adiciona.php?tipo=taxa";
			$taxa['titulo'] = "Inserir taxa de entrega";
			break;
		
		case 'atualizar' :
			$taxa['action'] = "atualiza.php?tipo=taxa&id=$id";
			$taxa['titulo'] = "Atualizar taxa de entrega";
			$sql = "SELECT txe_localizacao, txe_valor
			FROM {$tabela['taxa_entrega']}
			WHERE txe_cod = $id";	
			$resultado = $dataBase->query($sql);
			$dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC);
			$taxa['loc']    = $dados['txe_localizacao'];
			$taxa['valor']  = number_format($dados['txe_valor'],2,',','.');
			break;
	}
	...
	$template->setCurrentBlock("bloco_taxa");
		...
		/* Formulario */
			$template->setVariable("formTx", "form_tx");
			$template->setVariable("actionTx", $taxa['action']);
		
		/* Titulos */
			$template->setVariable("tituloTaxa", $taxa['titulo']);
	
		/* Nomes dos Campos */
			$template->setVariable("campoLoc", "localizacao");
			$template->setVariable("campoValor", "valor");

		/* Valores dos Campos */
			$template->setVariable("valorLoc", $taxa['loc']);
			$template->setVariable("valorValor", $taxa['valor']);
			
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaTx(form_tx.localizacao, form_tx.valor, form_tx)");
			$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
			
	$template->parseCurrentBlock("bloco_taxa");

	$show = $template->get();
	
	/* Título da Página Interna */
	$tituloInterna = "Taxas de entrega";
