...
/* Capturando Pedido */
$templateHtmlName = 'formEsqueciSenha.html';
...
$template->setCurrentBlock("bloco_esqueci_senha");
	...
	/* Formulario */
		$template->setVariable("formEsqueci", "form_esqueci");
		$template->setVariable("actionEsqueci", "enviaSenha.php");
	
	/* Titulos */
		$template->setVariable("tituloEsqueci", "Esqueceu sua senha ?");

	/* Nomes dos Campos */
		$template->setVariable("campoCpf", "cpf");
	
	/* Java Script ao Enviar */
		$template->setVariable("onClickEnviar", "enviaForm(form_esqueci)");
		$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
		
$template->parseCurrentBlock("bloco_esqueci_senha");

$show = $template->get();

/* Título da Página Interna */
$tituloInterna = "Recupere suas informações";
