...
if($permitido != true){
	...
	define(ASSUNTO, "feriados");
	define(TABELA, $tabela['feriados']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'fer_data');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CLASSIFICACAO);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT fer_cod, fer_nome, fer_data FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "feriado.php?acao=atualizar&id={$dados['fer_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=feriado&id={$dados['fer_cod']}");
			
			/* Linha a ser impressa */
				$nome = limitaStr($dados['fer_nome'], LIMITE_GENRENCIAR);
				$dataMostrar = converteData($dados['fer_data']);
				$mostra = $nome." (<b>".$dataMostrar."</b>)";
				$template->setVariable("linha", $mostra);
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Feriados");
