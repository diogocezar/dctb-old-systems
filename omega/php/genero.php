...
if($permitido != true){
	...
	switch($acao){
		case 'adicionar' :
			$genero['action'] = "adiciona.php?tipo=genero";
			$genero['titulo'] = "Inserir gênero";
			break;
		
		case 'atualizar' :
			$genero['action'] = "atualiza.php?tipo=genero&id=$id";
			$genero['titulo'] = "Atualizar gênero";
			$sql = "SELECT gen_nome, gen_descricao
			FROM {$tabela['genero']}
			WHERE gen_cod = $id";	
			$resultado = $dataBase->query($sql);
			$dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC);
			$genero['nome']        = $dados['gen_nome'];
			$genero['descricao']   = desconverteQuebra($dados['gen_descricao']);
			break;
	}
	...
	/* Capturando Pedido */
	$templateHtmlName = 'formGenero.html';
	...
	$template->setCurrentBlock("bloco_genero");
		...
		/* Formulario */
			$template->setVariable("formGenero", "form_genero");
			$template->setVariable("actionGenero", $genero['action']);
		
		/* Titulos */
			$template->setVariable("tituloGenero", $genero['titulo']);
		...
		/* Valores dos Campos */
			$template->setVariable("valorNome", $genero['nome']);
			$template->setVariable("valorDescricao", $genero['descricao']);
			
		/* BB Code */
			$template->setVariable("onClickNegrito", "wrapSelection(form_genero.descricao,'[B]','[/B]')");
			$template->setVariable("onClickItalico", "wrapSelection(form_genero.descricao,'[I]','[/I]')");
			$template->setVariable("onClickSublinhado", "wrapSelection(form_genero.descricao,'[U]','[/U]')");
			
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaGenero(form_genero.nome, form_genero.descricao, form_genero)");
			$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
			
	$template->parseCurrentBlock("bloco_genero");

	$show = $template->get();
	
	/* Título da Página Interna */
	$tituloInterna = "Gêneros";
