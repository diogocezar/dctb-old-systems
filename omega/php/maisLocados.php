...
define(ATUAL, $start);
define(POR_PAGINA, PP_MAIS_LOCADOS);
...
if($contem_resultado == false){
	$template->setCurrentBlock("bloco_gerenciar_erro");
			$template->setVariable("erro", "Não foi encontrado nenhum filme em '<b>Mais Locados</b>'.");
...
$template->setCurrentBlock("bloco_gerenciar");
	...
	/* Formulario */
		...
		$template->setVariable("actionBuscaOpcoes", "maisLocados.php");
	...
	/* Campos */
		...
		if($ordem == "qtd"){
			$template->setVariable("ordensOpcionais", "<option value=\"qtd\" selected>Mais Locados</option>");
		}
		else{
			$template->setVariable("ordensOpcionais", "<option value=\"qtd\">Mais Locados</option>");
		}
...
$tituloInterna = "Exibindo Todos os Filmes '<b>Mais Locados</b>'";
