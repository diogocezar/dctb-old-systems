...
if($permitido != true){
	...
	define(ASSUNTO, "gêneros");
	define(TABELA, $tabela['genero']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'gen_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_GENERO);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT gen_cod, gen_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "genero.php?acao=atualizar&id={$dados['gen_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=genero&id={$dados['gen_cod']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"mostraGenero.php?id={$dados['gen_cod']}\" target=\"_blank\">";
				$linha .= limitaStr($dados['gen_nome'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Gêneros");
