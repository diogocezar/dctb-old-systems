...
if($permitido != true){
	...
	switch($acao){
		...
		case 'atualizar' :
			...
			$sql = "SELECT pro_nome, pro_qtd, pro_preco
			...
			$produto['preco']   = number_format($dados['pro_preco'],2,',','.');
			break;
	}
	...
	$template->setCurrentBlock("bloco_produtos");
		...
		/* Formulario */
			...
			$template->setVariable("actionProdutos", $produto['action']);
		
		/* Titulos */
			$template->setVariable("tituloProdutos", $produto['titulo']);
		...
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaProdutos(form_produtos.nome, form_produtos.qtd, form_produtos.preco, form_produtos)");
