...
if($permitido != true){
	...
	switch($acao){
		...
		case 'atualizar' :
			...
			$sql = "SELECT cat_nome, cat_descricao, cat_temp_loc, cat_preco
			FROM {$tabela['categoria']}
			WHERE cat_cod = $id";	
			$resultado = $dataBase->query($sql);
			$dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC);
			$categoria['nome']        = $dados['cat_nome'];
			$categoria['descricao']   = desconverteQuebra($dados['cat_descricao']);
			$categoria['temp_loc']    = $dados['cat_temp_loc'];
			$categoria['preco']       = number_format($dados['cat_preco'],2,',','.');
			break;
	}
	...
	/* Capturando Pedido */
	$templateHtmlName = 'formCategoria.html';
	...
	$template->setCurrentBlock("bloco_categoria");
		...
		/* Nomes dos Campos */
			...
			$template->setVariable("campoTempLoc", "temp_loc");
		...
		/* Valores dos Campos */
			...
			$template->setVariable("valorTempLoc", $categoria['temp_loc']);
			$template->setVariable("valorPreco", $categoria['preco']);
		...
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaCategoria(form_categoria.nome, form_categoria.descricao, form_categoria.temp_loc, form_categoria.preco, form_categoria)");
			$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
			
	$template->parseCurrentBlock("bloco_categoria");

	$show = $template->get();
	
	/* Título da Página Interna */
	$tituloInterna = "Categorias";
