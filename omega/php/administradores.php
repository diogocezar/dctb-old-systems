...
if($permitido != true){
	...
	define(ATUAL, $start);
	define(POR_PAGINA, PP_ADMIN);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT usu_cod, usu_nome, usu_login FROM ".TABELA." WHERE tip_id_user > 1 ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "administrador.php?acao=atualizar&id={$dados['usu_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=admin&id={$dados['usu_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", limitaStr($dados['usu_nome']." (".$dados['usu_login'].") ", LIMITE_GENRENCIAR));
