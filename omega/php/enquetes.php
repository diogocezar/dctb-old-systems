...
if($permitido != true){
	...
	define(ASSUNTO, "enquetes");
	define(TABELA, $tabela['enquete']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'enq_pergunta');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_ENQUETE);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT enq_id, enq_pergunta FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "enquete.php?acao=atualizar&id={$dados['enq_id']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=enquete&id={$dados['enq_id']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"javascript:abrir('resultado.php?enquete={$dados['enq_id']}', '330', '319', 'no');\">";
				$linha .= limitaStr($dados['enq_pergunta'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Enquetes");
