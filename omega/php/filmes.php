...
if($permitido != true){
	...
	define(ATUAL, $start);
	define(POR_PAGINA, PP_FILMES);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT fil_cod, fil_titulo FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "filme.php?acao=atualizar&id={$dados['fil_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=filme&id={$dados['fil_cod']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"mostraFilme.php?id={$dados['fil_cod']}\" target=\"_blank\">";
				$linha .= limitaStr($dados['fil_titulo'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Filmes");
