...
if($permitido != true){
	...
	define(TABELA, $tabela['categoria']);
	...
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CATEGORIA);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT cat_cod, cat_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "categoria.php?acao=atualizar&id={$dados['cat_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=categoria&id={$dados['cat_cod']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"mostraCategoria.php?id={$dados['cat_cod']}\" target=\"_blank\">";
				$linha .= limitaStr($dados['cat_nome'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
