...
if($permitido != true){
	...
	$templateHtmlName = 'listaClientes.html';
	...
	define(ASSUNTO, "clientes");
	define(TABELA1, $tabela['cliente']);
	define(TABELA2, $tabela['usuario']);
	
	/* Contando registros */
	$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA1." c, ".TABELA2." u WHERE c.usu_cod = u.usu_cod");
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'usu_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_ATORES);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT u.usu_nome, u.usu_sobrenome, c.cli_cpf, c.cli_bloqueado FROM ".TABELA1." c, ".TABELA2." u WHERE c.usu_cod = u.usu_cod ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	$resultado = $dataBase->query(SQL);
	$contem_resultado = false;
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	
		$template->setCurrentBlock("bloco_lista_clientes_interno");
		
			/* Linha a ser impressa */
				$linha = limitaStr($dados['usu_nome']." ".$dados['usu_sobrenome'], LIMITE_GENRENCIAR);
				$template->setVariable("linkCliente", "javascript:abrir('mostraCliente.php?cliente={$dados['cli_cpf']}', '328', '443', 'no');");
				$template->setVariable("nome", $linha);
				
			/* Bloquear / Desbloquear */
			
			if($dados['cli_bloqueado'] == "Sim"){
				$template->setVariable("controle", "<a href = \"bloquear.php?id={$dados['cli_cpf']}&bloquear=Não\"><img src=\"../images/bot_desbloquear.jpg\" border =\"0\" alt=\"Desbloquear !\" /></a>");
			}
			else{
				$template->setVariable("controle", "<a href = \"bloquear.php?id={$dados['cli_cpf']}&bloquear=Sim\"><img src=\"../images/bot_bloquear.jpg\" border =\"0\" alt=\"Bloquear !\" /></a>");
			}
				
		$template->parseCurrentBlock("bloco_lista_clientes_interno");
		$contem_resultado = true;

	}
	/* Caso não ache nenhum resultado escreve para o usuário */
	if($contem_resultado == false){
		$template->setCurrentBlock("bloco_lista_clientes_erro");
				$template->setVariable("erro", "Não foi encontrado nenhum registro em '".ASSUNTO."'.");
		$template->parseCurrentBlock("bloco_lista_clientes_erro");
	}
	...
	if(!empty($paginacao)){
		$template->setCurrentBlock("bloco_lista_clientes_paginacao_controle");
					$template->setVariable("paginacao", $paginacao);
		$template->parseCurrentBlock("bloco_lista_clientes_paginacao_controle");
	}
	
	if($contem_resultado == true){
		$template->setCurrentBlock("bloco_lista_clientes_paginacao");
					$template->setVariable("infos", $infos);
		$template->parseCurrentBlock("bloco_lista_clientes_paginacao");
	}
	
	$template->setCurrentBlock("bloco_lista_clientes");
	
		/* Titulos */
			$template->setVariable("tituloClientes", "Clientes");
	...
	$template->parseCurrentBlock("bloco_lista_clientes");
	
	$show  = $template->get();
	$show .= "<br>";
	
	/* Título da Página Interna */
	$tituloInterna = "Clientes";
