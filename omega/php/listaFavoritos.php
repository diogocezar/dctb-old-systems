...
if($permitido != true){
	...
	$qtd = $dataBase->getOne("SELECT count(f.fil_titulo)
							  FROM {$tabela['favoritos']} fa, {$tabela['cliente']} c, {$tabela['filme']} f
							  WHERE c.cli_cpf = fa.cli_cpf AND f.fil_cod = fa.fil_cod AND c.cli_cpf = '$codUser'");
	...
	define(SQL, "SELECT DISTINCT f.fil_titulo, f.fil_foto, f.fil_cod
				 FROM {$tabela['favoritos']} fa, {$tabela['cliente']} c, {$tabela['filme']} f
				 WHERE c.cli_cpf = fa.cli_cpf AND f.fil_cod = fa.fil_cod AND c.cli_cpf = '$codUser'
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_detalhes_capa_interno");
			...
			/* Detalhes */
				$detalhes = "<a href=\"removeFavorito.php?id={$dados['fil_cod']}\">Remover Favorito</a>";
				if(!locado($dados['fil_cod'])){
					$detalhes .= " - <a href=\"carrinho.php?id={$dados['fil_cod']}&tipo=filme\">Locar";
				}
	...
	if($contem_resultado == false){
		$template->setCurrentBlock("bloco_detalhes_capa_erro");
				$template->setVariable("erro", "Não foi encontrado nenhum filme em sua lista de favoritos.");
	...
	$template->setCurrentBlock("bloco_detalhes_capa");
		...
		/* Titulos */
			$template->setVariable("tituloDetalhes", "Listagem de todos os filmes favoritos");
	...
	$tituloInterna = "Filmes Favoritos";
