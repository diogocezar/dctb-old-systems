...
if($permitido != true){
	...
	define(ASSUNTO, "tipos de usuário");
	define(TABELA, $tabela['tipo_user']);
	
	/* Contando registros */
	$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." WHERE tip_id_user > 1");
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'tip_tipo');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_TIPOS);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT tip_id_user, tip_tipo FROM ".TABELA." WHERE tip_id_user > 1 ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "tipo.php?acao=atualizar&id={$dados['tip_id_user']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=tipoUser&id={$dados['tip_id_user']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", limitaStr($dados['tip_tipo'], LIMITE_GENRENCIAR));
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Tipos de Usuário");
