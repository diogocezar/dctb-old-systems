...
if($permitido != true){
	...
	switch($acao){
		case 'adicionar' :
			$novidade['action'] = "adiciona.php?tipo=novidade";
			$novidade['titulo'] = "Inserir novidade";
			break;
		
		case 'atualizar' :
			$novidade['action'] = "atualiza.php?tipo=novidade&id=$id";
			$novidade['titulo'] = "Atualizar novidade";
			$sql = "SELECT nov_titulo, nov_conteudo
			FROM {$tabela['novidade']}
			WHERE nov_id = $id";	
			$resultado = $dataBase->query($sql);
			$dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC);
			$novidade['titulo_nov'] = $dados['nov_titulo'];
			$novidade['conteudo']   = desconverteQuebra($dados['nov_conteudo']);
			break;
	}
	...
	/* Capturando Pedido */
	$templateHtmlName = 'formNovidade.html';
	...
	$template->setCurrentBlock("bloco_novidade");
		...
		/* Formulario */
			$template->setVariable("formNovidade", "form_novidade");
			$template->setVariable("actionNovidade", $novidade['action']);
		
		/* Titulos */
			$template->setVariable("tituloNovidade", $novidade['titulo']);
		...
		/* Valores dos Campos */
			$template->setVariable("valorTitulo", $novidade['titulo_nov']);
			$template->setVariable("valorConteudo", $novidade['conteudo']);
			
		/* BB Code */
			$template->setVariable("onClickNegrito", "wrapSelection(form_novidade.conteudo,'[B]','[/B]')");
			$template->setVariable("onClickItalico", "wrapSelection(form_novidade.conteudo,'[I]','[/I]')");
			$template->setVariable("onClickSublinhado", "wrapSelection(form_novidade.conteudo,'[U]','[/U]')");
			
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaNovidade(form_novidade.titulo, form_novidade.conteudo, form_novidade)");
			$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
			
	$template->parseCurrentBlock("bloco_novidade");
