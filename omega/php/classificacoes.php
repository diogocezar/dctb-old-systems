...
if($permitido != true){
	...
	define(ASSUNTO, "classificações");
	define(TABELA, $tabela['classificacao']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'cla_classificacao');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CLASSIFICACAO);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT cla_cod, cla_classificacao, cla_idade_recomendada FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "classificacao.php?acao=atualizar&id={$dados['cla_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=classificacao&id={$dados['cla_cod']}");
			
			/* Linha a ser impressa */
				$linha  = "<a href = \"mostraClassificacao.php?id={$dados['cla_cod']}\" target=\"_blank\">";
				$linha .= limitaStr($dados['cla_classificacao'], LIMITE_GENRENCIAR);
				$linha .= "</a>";
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Classificações");
