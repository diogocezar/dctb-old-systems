...
if($permitido != true){
	...
	switch($acao){
		case 'adicionar' :
			$feriado['action'] = "adiciona.php?tipo=feriado";
			$feriado['titulo'] = "Inserir feriado";
			break;
		
		case 'atualizar' :
			$feriado['action'] = "atualiza.php?tipo=feriado&id=$id";
			$feriado['titulo'] = "Atualizar feriado";
			$sql = "SELECT fer_data, fer_nome
			FROM {$tabela['feriados']}
			WHERE fer_cod = $id";	
			$resultado = $dataBase->query($sql);
			$dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC);
			$feriado['nome'] = $dados['fer_nome'];
			$feriado['data'] = converteData($dados['fer_data']);
			break;
	}
	...
	/* Capturando Pedido */
	$templateHtmlName = 'formFeriado.html';
	...
	$template->setCurrentBlock("bloco_feriado");
		...
		/* Formulario */
			$template->setVariable("formFeriado", "form_feriado");
			$template->setVariable("actionFeriado", $feriado['action']);
		
		/* Titulos */
			$template->setVariable("tituloFeriado", $feriado['titulo']);
		...
		/* Valores dos Campos */
			$template->setVariable("valorNome", $feriado['nome']);
			$template->setVariable("valorData", $feriado['data']);
			
		/* Java Script ao Enviar */
			$template->setVariable("onClickEnviar", "validaFeriado(form_feriado.nome, form_feriado.data, form_feriado)");
			$template->setVariable("onClickVoltar", "javascript:javascript:history.go(-1);");
			
	$template->parseCurrentBlock("bloco_feriado");

	$show = $template->get();
	
	/* Título da Página Interna */
	$tituloInterna = "Feriados";
