...
$templateHtmlName = 'produtos.html';
...
/* Contando registros */
$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." WHERE pro_qtd > 0");
...
define(SQL, "SELECT pro_nome, pro_preco, pro_cod, pro_qtd FROM ".TABELA." WHERE pro_qtd > 0 ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
...
while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
	...
	$template->setCurrentBlock("bloco_gerenciar_interno");
		...
		/* Campo Qtd */
			$template->setVariable("qtdProdutos", "qtd_{$dados['pro_cod']}");
			
		/* Campo QtdEstoque */
			$template->setVariable("qtdEstoque", "qtd_est_{$dados['pro_cod']}");
			$template->setVariable("valorQtdEstoque", $dados['pro_qtd']);
		...
		/* Produto */
			...
			if($_SESSION['usuarioSession'] != 'sim'){
				$logado  = false;
				$link    = "javascript:alert('Você prescisa estar logado no site comprar um produto.')";
			}
			else{
				$logado  = true;
				$link = "'carrinho.php?id={$dados['pro_cod']}&tipo=produto'";
			}
		
			$template->setVariable("produto", $dados['pro_nome']);
			$template->setVariable("valor", number_format($dados['pro_preco'],2,',','.'));
			$template->setVariable("actionProdutos", "#");
			$template->setVariable("formProdutos", "form_produtos");
			if($logado == true){
				$template->setVariable("onClickComprar", "addProduto(form_produtos.qtd_{$dados['pro_cod']}, form_produtos.qtd_est_{$dados['pro_cod']}, $link)");
			}
			else{
				$template->setVariable("onClickComprar", "$link");
			}
...
$tituloInterna = "Exibindo os Produtos Disponíveis";
