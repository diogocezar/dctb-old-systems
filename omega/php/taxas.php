...
if($permitido != true){
	...
	define(ASSUNTO, "taxas de entrega");
	define(TABELA, $tabela['taxa_entrega']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'txe_localizacao');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_TAXAS);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT txe_cod, txe_localizacao FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "taxa.php?acao=atualizar&id={$dados['txe_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=taxa&id={$dados['txe_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", limitaStr($dados['txe_localizacao'], LIMITE_GENRENCIAR));
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("tituloGerenciar", "Gerenciar Taxas de Entrega");
