...
if($permitido != true){
	...
	define(ASSUNTO, "cursos");
	define(TABELA, $tabela['cursos']);
	
	/* Contando registros */
	$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA."  WHERE cur_cod <> 1");
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'cur_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CURSOS);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	define(SQL, "SELECT cur_cod, cur_nome FROM ".TABELA." WHERE cur_cod <> 1 ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "curso.php?acao=atualizar&id={$dados['cur_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=curso&id={$dados['cur_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"mostracurso.php?id={$dados['cur_cod']}\" class=\"link_escuro\">".limitaStr(str_replace("Curso de Especialização em ", "", $dados['cur_nome']), 55)."</a>");
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("titulo", "Gerenciar Cursos");
