...
/* Fazendo a verificação na sessão para averiguar se o usuário é permitido */
...
if($permitido != true){
	...
	define(ASSUNTO, "contatos");
	define(TABELA, $tabela['contato']);
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'con_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_CONTATOS);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	if($curso == 1){ // se for o administrador
		define(SQL, "SELECT con_cod, con_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	else{
		define(SQL, "SELECT con_cod, con_nome FROM ".TABELA." WHERE cur_cod = $curso ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "contato.php?acao=atualizar&id={$dados['con_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=contato&id={$dados['con_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"contato.php?acao=atualizar&id={$dados['con_cod']}\" class=\"link_escuro\">".limitaStr($dados['con_nome'], 55)."</a>");
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("titulo", "Gerenciar Contatos");
