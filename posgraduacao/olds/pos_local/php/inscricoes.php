...
if($permitido != true){
	...
	define(ORDEM, 'ASC');
	define(ORDENADO, 'ins_nome');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_INSCRICOES);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	if($curso == 1){ // se for o administrador
		define(SQL, "SELECT ins_cod, ins_nome FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	else{
		define(SQL, "SELECT ins_cod, ins_nome FROM ".TABELA." WHERE cur_cod = $curso ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"inscricao.php?acao=atualizar&id={$dados['ins_cod']}\" class=\"link_escuro\">".limitaStr($dados['ins_nome'], 55)."</a>");
