...
function validaAlterar(senha, nova_senha, confima_senha, form){
	...
	if(confima_senha.value == ""){
		alert(retornaString('confirmação de senha'));
...
function validaCurso(nome, inicio, termino, periodo_insc, periodo_matr, turno, vagas, taxa, matricula, mensalidade, apresentacao, objetivos, certificado, resumo, form){
	...
	if(inicio.value == ""){
		alert(retornaString('início do curso'));
		setaFoco(inicio);
		erro = true;
	}
	if(termino.value == ""){
		alert(retornaString('término do curso'));
		setaFoco(termino);
		erro = true;
	}
	if(periodo_insc.value == ""){
		alert(retornaString('período de inscrição'));
		setaFoco(periodo_insc);
		erro = true;
	}
	if(periodo_matr.value == ""){
		alert(retornaString('período de matrícula'));
	...
	if(taxa.value == ""){
		alert(retornaString('taxa de inscrição'));
		setaFoco(taxa);
		erro = true;
	}
	if(matricula.value == ""){
...
function validaInscricao(nome, cpf, rg, orgaoEmissor, data_nascimento, graduacao, rua, numero, bairro, cidade, cep, telefone, email, form){
	var erro = false;
	...
	if(orgaoEmissor.value == ""){
		alert(retornaString('orgão emissor'));
