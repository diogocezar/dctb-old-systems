...
if($permitido != true){
	...
	define(ASSUNTO, "inscrições");
	define(TABELA, $tabela['inscricoes']);
	...
	define(ORDEM, 'DESC');
	define(ORDENADO, 'ins_cod');
	define(ATUAL, $start);
	define(POR_PAGINA, PP_INSCRICOES);
	define(TOTAL, ceil((QTD)/POR_PAGINA));
	if($curso == 1){ // se for o administrador
		define(SQL, "SELECT ins_cod, ins_nome, ins_quando FROM ".TABELA." ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	else{
		define(SQL, "SELECT ins_cod, ins_nome, ins_quando FROM ".TABELA." WHERE cur_cod = $curso ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "inscricao.php?acao=atualizar&id={$dados['ins_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=inscricao&id={$dados['ins_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"inscricao.php?acao=atualizar&id={$dados['ins_cod']}\" class=\"link_escuro\">".limitaStr($dados['ins_nome'], 55)."</a> - <b>".extraiData($dados['ins_quando'])."</b>");
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("titulo", "Gerenciar Inscrições");
