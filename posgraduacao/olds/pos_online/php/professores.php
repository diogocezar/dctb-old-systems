...
if($permitido != true){
	...
	if($curso == 1){ // se for o administrador
		...
		$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." prof, disciplinas dic WHERE dic.cur_cod = $curso AND prof.pro_cod = dic.pro_cod");
	}
	...
	define(ATUAL, $start);
	define(POR_PAGINA, PP_PROFESSORES);
	...
	if($curso == 1){ // se for o administrador
		...
		define(SQL, "SELECT prof.pro_cod, prof.pro_nome FROM ".TABELA." prof, disciplinas dic WHERE dic.cur_cod = $curso AND prof.pro_cod = dic.pro_cod ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	}
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Links da imagens dos Campos */
				$template->setVariable("linkEditar", "professor.php?acao=atualizar&id={$dados['pro_cod']}");
				$template->setVariable("linkExcluir", "exclui.php?tipo=professor&id={$dados['pro_cod']}");
			
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"mostraprofessor.php?id={$dados['pro_cod']}\" class=\"link_escuro\">".limitaStr($dados['pro_nome'], 55)."</a>");
	...
	$template->setCurrentBlock("bloco_gerenciar");
		...
		/* Titulos */
			$template->setVariable("titulo", "Gerenciar Professores");
