...
if($permitido != true){
	...
	switch($op){
		case 'curso' :
			...
			$atualizar->Update($tabela['cursos'], "cur_cod = $id", $campos['cursos'], $valores);
		...
		case 'noticia' :
			...
			$atualizar->Update($tabela['noticias'], "not_cod = $id", $campos['noticias'], $valores);
		...
		case 'inscricao' :
			...
			$atualizar->Update($tabela['inscricoes'], "ins_cod = $id", $campos['inscricoes'], $valores);
			...
			if(!empty($idAdm)){
				...
				if(!empty($destino)){
					...
					$origem    = "depog-cp@utfpr.edu.br.br";
		...
		case 'professor' :
			...
			$atualizar->Update($tabela['professores'], "pro_cod = $id", $campos['professores'], $valores);
		...
		case 'disciplina' :
			...
			$atualizar->Update($tabela['disciplinas'], "dic_cod = $id", $campos['disciplinas'], $valores);
		...
		case 'contato' :
			...
			$atualizar->Update($tabela['contato'], "con_cod = $id", $campos['contato'], $valores);
		...
		case 'alterar_senha':
			...
			/* Resgatando senha atual do banco */
			$sql = "SELECT adm_senha FROM {$tabela['administradores']} WHERE adm_cod = 1";	
			...
			if($senhaAtual != $senha){
				echo "<script language=javascript>alert('A senha atual digitada não confere com a senha armazenada');javascript:history.go(-1)</script>";
			}
			else{
				$dataBase->query("UPDATE {$tabela['administradores']} SET adm_senha = '$senhaNova' WHERE adm_cod = 1");
			}			
			break;
	}
	...
	$msg .= "<br><br>";
	$msg .= "<img src=\"../icones/agt_reload.jpg\"><br><br>";
