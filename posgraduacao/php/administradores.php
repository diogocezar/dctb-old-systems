...
if($permitido != true){
	...
	/* Contando registros */
	$qtd = $dataBase->getOne("SELECT count(*) FROM ".TABELA." WHERE adm_cod <> 1");
	...
	define(SQL, "SELECT adm_cod, adm_nome FROM ".TABELA." WHERE adm_cod <> 1 ORDER BY ".ORDENADO." ".ORDEM." LIMIT ".ATUAL.", ".POR_PAGINA);
	...
	while($dados = $resultado->fetchRow(DB_FETCHMODE_ASSOC)){
		$template->setCurrentBlock("bloco_gerenciar_interno");
			...
			/* Linha a ser impressa */
				$template->setVariable("linha", "<a href = \"administrador.php?acao=atualizar&id={$dados['adm_cod']}\" class=\"link_escuro\">".limitaStr($dados['adm_nome'], 55)."</a>");
