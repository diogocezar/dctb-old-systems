...
#require_once("Errors.php");
	  ...
	  * 2. As imagens devem conter apenas as seguintes extenções : jpg, png, gif
	  * 3. As extenções devem ser escritas em letras minúsculas, exemplo bola.jpg
	  */
...
class Pikture{
	...
	function printFolders($pagMostra, $varGet, $iconePastinha, $colunasCapa){
		...
		for($i=0; $i<count($array); $i++){
			...
			$link = "<a href = \"$pagMostra?$varGet={$array[$i]}&id=".$_GET['id']."\" class=\"link_cinza\">";
			$fechaLink = "</a>";
	...
	function showPhotos($subDir, $page, $scalar, $printFileName, $printSize){
		...
		for($i=0; $i<$linhas; $i++){
			...
			for($j=0; $j<$colunas; $j++){
				...
				$retorno .= "<a href=\"#\" onClick=\"abrir('mostra.php?loc=".$dirFoto."&a=$altura&l=$largura&titulo=".$nome."', '".$largura."', '".$altura."', 'no');\" class=\"link_cinza\">";
	...
	function getSize($url){
	 ...
	 /** 
	  * Método retorna imprime/retorna o estilo a partir da tag passada como parâmetro.
	  * @access public 
	...
	function printPaging($pag, $paginaAtual, $iconePrimeiro, $iconeAnterior, $iconeProximo, $iconeUltimo){
		...
		$paginacao .= "<a href=\"".PAGINA_ATUAL."?folder=".SESSAO."&id=".$_GET['id']."\" class=\"link_cinza\"><img src=\"$iconePrimeiro\" border=\"0\"><a>&nbsp;&nbsp;";
		if($pag > 0){
			$paginacao .= "<a href=\"".PAGINA_ATUAL."?page=$pag_ante&folder=".SESSAO."&id=".$_GET['id']."\" class=\"link_cinza\">";
		}		
		...
		for($i=$inicioMostra; $i<=$fimMostra ; $i++){
			if($i == ATUAL+1){	
				...
				$aevi = POR_PAGINA*($i-1);
				$mI = $i-1;
				$paginacao .= " <a href=\"".PAGINA_ATUAL."?page=$mI&folder=".SESSAO."&id=".$_GET['id']."\" class=\"link_cinza\">$i</a> ";
			}
		}
		
		$aevi = POR_PAGINA*(TOTAL-1);
		$i    = TOTAL-1;
		$link = PAGINA_ATUAL."?page=$i&folder=".SESSAO."&id=".$_GET['id'];
		
		$paginacao .= "&nbsp;<b>.</b>.&nbsp;";
		
		if($pag+1 < TOTAL){						
			$paginacao .= "<a href=\"".PAGINA_ATUAL."?page=$pag_prox&folder=".SESSAO."&id=".$_GET['id']."\" class=\"link_cinza\">";
		}
		
		$paginacao .= "<img src=\"$iconeProximo\" border=\"0\"></a>&nbsp;";
		$paginacao .= "<a href=\"$link\" class=\"link_cinza\"><img src=\"$iconeUltimo\" border=\"0\"></a>";									
	...
	function printVoltar($iconeVoltar, $linkPrincipal){
		$retorno .= "<br><br>";
		$retorno .= "<div align=\"center\">";
		$retorno .= "<input name=\"btnVoltar\" type=\"button\" class=\"botao\" id=\"btnVoltar\" value=\"« Voltar\" onclick=\"javascript:location.href='$linkPrincipal'\">";
		$retorno .= "</div><br>";
