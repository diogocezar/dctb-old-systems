...
if(!empty($id)){
	...
	$template->setCurrentBlock("bloco_topo");
		...
		$alterarSenha = "<a href=\"javascript:;\" onclick=\"tb_show('Alterar a sua senha:', 'frmAlterarSenha.php?height=200&width=250', '');\" class=\"linkBranco\">Alterar Senha</a>";
		$ajuda        = "<a href=\"ajuda.php\" class=\"linkBranco\">Ajuda</a>";
		$sair         = "<a href=\"sair.php\" class=\"linkBranco\">Sair</a>";
		$template->setVariable("login", "Bem vindo, $nome - <strong>$descricaoNivel</strong> (".$alterarSenha.")");
...
if($vencimentosDiarios){
	/* bloco conteudo contas */
	$template->setCurrentBlock("bloco_conteudo");
		$template->setVariable("titulo", 'Vencimentos diários');
	$template->parseCurrentBlock("bloco_conteudo");
}

if($carregaRelatorios){
	/* bloco relatorios */
	$template->setCurrentBlock("bloco_conteudo_relatorio");
		$template->setVariable("conteudoRelatorio", "<img src=\"../images/loadingAnimation.gif\"> Aguarde, carregando...");
