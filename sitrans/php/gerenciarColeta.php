...
$template->setCurrentBlock("bloco_html");
	...
	$onLoad .= "onLoad = \"";
	$onLoad .= "call_getFiltredRegistersColeta(true, event);";
	...
	$template->setVariable("cmp_form_key_press", "call_getFiltredRegistersColeta(false, event)");
	...
	if(is_array($campos)){
		...
		$camposReplace = 'nome do cliente, '.$camposReplace;
		$camposReplace = 'cidade do cliente, '.$camposReplace;
		if($opcao == 'roteirizar'){
			$camposReplace = 'placa do veículo, '.$camposReplace;
		}
