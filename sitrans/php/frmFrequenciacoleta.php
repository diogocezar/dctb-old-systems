...
$template->setCurrentBlock("bloco_formulario");
	...
	/* nomes dos campos */
		...
		$template->setVariable("campoQtdadedias", "qtdadedias");
		$template->setVariable("campoSituacao",   "situacao");
	
	/* valores dos campos */
		$template->setVariable("valorDescricao",   $objRec->getDescricao());
		$template->setVariable("valorQtdadedias",  $objRec->getQtdadedias());
	...
	/* foco no campo inicial */
		$onLoad .= "onLoad = \"";
		$onLoad .= "document.$form.descricao.focus();";
		$onLoad .= "\"";
