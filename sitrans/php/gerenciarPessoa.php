...
$template->setCurrentBlock("bloco_html");
	...
	$onLoad .= "onLoad = \"";
	$onLoad .= "call_getFiltredRegistersPessoa(true, event);";
	...
	$template->setVariable("cmp_form_key_press", "call_getFiltredRegistersPessoa(false, event)");
	...
	if(is_array($campos)){
		foreach($campos as $campoAtual){
			...
			$camposReplace .= $aliasMap[$tabelaCampo][$campoAtual];
			$i++;
			if($i != count($campos)){
				$camposReplace .= ", ";
			}
