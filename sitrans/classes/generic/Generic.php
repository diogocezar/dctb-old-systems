...
* SpeceBraid
*
* Essa classe é uma classe genérica, que é extendida pelas demais classes do sistema
* assim, todos herdeiros terão essas funcionalidades
*
...
abstract class Generic{
	...
	* @return int
	*/
    public function max_r($type = 'curval'){
		...
		if($type != 'curval'){
			...
			$sql = "SELECT CURRVAL('".$this->getTabela()."_".$key."_seq') as maximo";			
		}
	...
	/** 
	* Método inicia uma transação no banco de dados
	* @access public
	*/
    public function startTransaction(){		
		...
		$sql .= "LOCK TABLE ".$this->getTabela()." IN SHARE MODE;";
		...
		if(DB::isError($resultado)){
			$this->erro($erro['ERRO_TRAN_START'].$resultado->getMessage());
		}
    }//startTransaction
	
	/** 
	* Método finaliza uma transação no banco de dados
	* @access public
	*/
    public function commitTransaction(){		
		...
		if(DB::isError($resultado)){
			$this->erro($erro['ERRO_TRAN_COMIT'].$resultado->getMessage());
		}
    }//startTransaction
	...
	*/
	...
	/** 
	* Método que abstrato que será implementado nas classes herdeiras, esse método deve
