...
<table width="763" height="768" border="0" align="center" cellpadding="0" cellspacing="0" id="Table_01">
  ...
  <tr>
    ...
    <?php
		...
		switch($tipo){
			case 'noticia':
				...
				$tit  = $_POST['not_titulo'];
				$font = $_POST['not_fonte'];
				$desc = $_POST['not_descricao'];
				
				$idAutor = retornaIdSess($id);
				...
				$MySQL->Insert($tabela['noticias'], $camposTab1, $ins);
			...
			case 'admin':
				...
				if(!checkMail($email)){
					echo "<script language=javascript>alert('Desculpe o e-mail digitado não é válido');location.href='frm_ins_adm.php'</script>";	
				}
				...
				$MySQL->Insert($tabela['admin'], $camposTab2, $ins);
				
				$msg = "Administrador inserido com sucesso !!!";					
			break;
			case 'par':
				$titulo = 'Sites Realacionados';
				
				$link      = $_POST['par_link'];
				$descircao = $_POST['par_descricao'];
				...
				$MySQL->Insert($tabela['parceiro'], $camposTab3, $ins);
				
				$msg = "Site relacionado inserido com sucesso !!!";						
				break;
		}
	...
	<div align="center" class="style13">
      <p>&nbsp;</p>
      <p><img src="images/bolinha.gif" width="6" height="6"> Adicionando 
        <?= $titulo ?>
      </p>
	</div>
		<span class="style14"><?= $msg ?></span>
	  <p>
